...
<div id="menu"> 
    <div class="menu_l">当前位置：<a href="#">系统管理</a> > <a href="#">用户权限</a></div> 
    <div class="menu_r">
  ...
  <div class="main_r1">
	...
	<form action="<?php echo HIGHPHP_WWW_HOST;?>account/userpoweredit.html" id="applyForm" method="post" >
      <table  border="0" cellspacing="0" cellpadding="0" class="sum_table3" id="table1" >
       
			
		
		<tr>   
          <th colspan="2" >给（<?php echo $user_info['username'];?>）用户分配权限（选择角色，会给用户分配对应角色相关默认权限）</th>        
        </tr> 
		
		  <tr>
			<td colspan="2">		
		<?php foreach ($role_list as $key => $row) {?> 
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;			<a href="<?php echo HIGHPHP_WWW_HOST; ?>account/userpower?id=<?php echo $user_id; ?>&role_id=<?php echo $key; ?>"
	<?php if ($key == $role_id) { ?>style="color:#ff6600;"<?php } ?>
><?php echo $row; ?></a>
			<?php } ?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a href="<?php echo HIGHPHP_WWW_HOST; ?>account/userpower?id=<?php echo $user_id; ?>" 
				<?php if (!$role_id) { ?>style="color:#ff6600;"<?php } ?>>无角色</a>
		...
		<?php if (isset($user_id) && $user_id!='') {?>
		  <?php foreach($power_list as $key => $row) { ?> 
			...
			<?php foreach ($row as $child_row) { ?>
				...
				<?php if (isset($user_power[$child_row['id']]) && 1== $user_power[$child_row['id']]) {
