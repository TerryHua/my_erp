...
<body>
<div id="menu"> 
    <div class="menu_l">当前位置：<a href="#">信息管理</a> > <a href="#">新闻管理</a></div> 
    <div class="menu_r">
    <a href="javascript:void(0)" onclick="WinOpen('<?php echo HIGHPHP_WWW_HOST; ?>info/newsadd.html')"><img src="<?php echo HIGHPHP_WWW_HOST; ?>images/menu_add.png" />添加新闻</a>
...
<select name="status" id="select"  class="s_select">
        ...
        <option value="0" <?php if ($condition['status'] === '0') { echo 'selected'; } ?>>未发布</option>
        <option value="1"  <?php if ($condition['status'] == '1') { echo 'selected'; } ?> >已发布</option>
      </select>
  ...
  <div class="main_r1">
    <div class="sheet2">
      ...
      <td><?php if ($v['status'] == '0') { echo '未发布';} else { echo "已发布";} ?></td>    

      <td>
      <a href="javascript:void(0)" onclick="WinOpen('<?php echo HIGHPHP_WWW_HOST; ?>info/newsedit?id=<?php echo $v['id']; ?>')">修改</a>
      <a href="<?php echo HIGHPHP_WWW_HOST; ?>info/newsdel?id=<?php echo $v['id']; ?>"   onclick="if(!confirm('确定删除?'))return false;">删除</a>
        </td>     
		</tr>
